...
int main() {
	...
	variable_t chr_par1_data = {.string_t = "AbBdefghijklmno"};
	elem_t *chr_par1 = create_var_item("chr1par1", VAR_STRING, chr_par1_data, true, false);

	variable_t chr_par2_data = {.int_t = 2};
	...
	elem_t *ord = create_function("ord", params->symbol.sym_var_list, returns->symbol.sym_var_list);
	...
	add_instruction(IC_STR2INT_VAR, ord, NULL, NULL);
